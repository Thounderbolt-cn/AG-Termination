...
public class DateUtils {
    ...
    private static final String[] dateFormatall = { "yyyy-MM-dd", "yyyy-MM-dd HH:mm:ss", "yyyy/MM/dd HH:mm:ss", "yyyyMMdd HH:mm:ss",
            ...
            "yyyy.MM.dd", "yy.MM.dd", "yyyy.MM", "yy.MM", "yyyy年MM月" , "yyyy年MM月dd日 HH:mm:ss","yyyy-MM","yyyyMM"};
    ...
    public static void main(String[] args) {
        Date d = DateUtils.minuteAfter(new Date(), -5);
        System.out.println(d);
    }

    private static final Map<Pattern, String> dateRegFormat = PatternUtil.dateRegFormat;
...
//	}
        ...
        try {
            for (Pattern key : dateRegFormat.keySet()) {
                if (key.matcher(dateStr).matches()) {
                    ...
                    String[] dateStringArray = dateStr.split("-");
                    if (dateStringArray.length > 2) {
                        String date = dateStringArray[2];
                        if (date.length() >= 2 && NumberUtils.isNumber(date.substring(0, 2))) {
                            if (Integer.parseInt(dateStringArray[1]) > 12 || Integer.parseInt(date.substring(0, 2)) > 31) {
                                return "";
                            }
                        }
                    }
     ...
     * @return
     */
    public static Date getCalYearDate(Date date, Integer years) {
     ...
     * @return
     */
    public static Date getCalYearFirstDate(Date date,Integer years) {
